...
REVIEW_MODE_BOTH = 'both'


def normalize_review_mode(mode: Optional[str]) -> str:
    """
    Normalize review mode to supported values.

    Args:
        mode: Review mode string

    Returns:
        Normalized mode (REVIEW_MODE_MISSING or REVIEW_MODE_BOTH)
    """
    if mode in (REVIEW_MODE_MISSING, REVIEW_MODE_BOTH):
        return mode
    return REVIEW_MODE_BOTH
...
def load_session_stats(raw: Any) -> dict:
    ...
    if raw:
        ...
        if isinstance(source, dict):
            ...
            review_mode = source.get('review_mode')
            stats['review_mode'] = normalize_review_mode(review_mode)
