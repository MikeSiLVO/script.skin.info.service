...
def merge_ratings(
    ...
    Direct source APIs (TMDB, Trakt, OMDb) are preferred over aggregators (MDBList)
    when they provide the same rating source, even if aggregator has more votes.
...
def prepare_kodi_ratings(
    ...
    for source_name, rating_data in merged_ratings.items():
        ...
        if not (0.0 <= rating <= 10.0):
            xbmc.log(
                f"SkinInfo [Ratings]: CRITICAL - Rating out of valid range for '{source_name}': {rating:.2f} "
