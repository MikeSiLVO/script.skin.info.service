"""Generic worker queue base class for multi-threaded background processing."""
...
class WorkerQueue:
    ...
    def stop(self, wait: bool = True) -> None:
        ...
        for i, worker in enumerate(self.workers):
            ...
            if worker.is_alive():
                xbmc.log(
                    f"SkinInfo: {self.__class__.__name__} worker {i} still alive after join timeout",
                    xbmc.LOGWARNING
                )
    ...
    def add_item(self, item: Any, dedupe_key: Optional[Any] = None) -> bool:
        ...
        if not self.running:
            xbmc.log(
                f"SkinInfo: {self.__class__.__name__} cannot add item, queue not started",
                xbmc.LOGWARNING
            )
            return False
    ...
    def wait(self, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            xbmc.log(
                f"SkinInfo: {self.__class__.__name__} wait error: {str(e)}",
                xbmc.LOGERROR
            )
            return False
        ...
        while self.running:
            ...
            try:
                ...
                try:
                    ...
                    xbmc.log(
                        f"SkinInfo: {self.__class__.__name__} worker {worker_id} error: {str(e)}",
                        xbmc.LOGWARNING
                    )
                ...
                xbmc.log(
                    f"SkinInfo: {self.__class__.__name__} worker {worker_id} unexpected error: {str(e)}",
                    xbmc.LOGERROR
                )
